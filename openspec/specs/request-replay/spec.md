# request-replay Specification

## Purpose
Re-issue a captured (optionally edited) request from the collector and record the outcome as a first-class capture entry, so "change one thing and see what happens" stays inside the inspector.
